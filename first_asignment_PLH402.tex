...
\sol{
	...
	\begin{center}
		\begin{tikzpicture}[->,>=stealth',shorten >=0pt,node distance=2cm,auto]
			\en
			\node[state,initial] (q1)				{q$1$};  % this is on the top left, and it is a starting state
			\node[state, accepting] (q2) [right of = q1] {q$2$};  % define where the other nodes will be presented
			\node[state] (q3) [right of = q2] {q$3$};  % accepting means final state.
			\node[state] (q4) [below of = q2] {q$4$};
			\node[state] (q5) [below of = q3] {q$5$};
			\node[state,accepting] (q6) [below of = q4] {q$6$};
